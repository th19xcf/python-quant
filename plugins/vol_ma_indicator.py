...
class VolMAIndicatorPlugin(IndicatorPlugin):
    ...
    def __init__(self):
        ...
        self.description = "成交量MA指标插件，通过IndicatorManager统一计算"
    ...
    def calculate(self, data, **kwargs):
        ...
        try:
            ...
            result_df = global_indicator_manager.calculate_indicator(
                data, 'vol_ma', return_polars=False, windows=windows
            )
            
            logger.debug(f"成交量MA指标插件通过IndicatorManager计算完成，窗口: {windows}")
            return result_df
        except Exception as e:
            logger.exception(f"成交量MA指标插件计算失败: {e}")
            raise
    ...
    def calculate_polars(self, data, **kwargs):
        ...
        try:
            ...
            result_df = global_indicator_manager.calculate_indicator(
                data, 'vol_ma', return_polars=True, windows=windows
            )
            
            logger.debug(f"成交量MA指标插件通过IndicatorManager计算完成，窗口: {windows}")
            return result_df
        except Exception as e:
            logger.exception(f"成交量MA指标插件Polars计算失败: {e}")
            raise
